...
import (
	"osm/api/models"
	service "osm/api/service/auth_service"
...
func (h authHandler) Old_login(c *fiber.Ctx) error {
	var login_body models.Login_body
	if err := c.BodyParser(&login_body); err != nil {
		return c.Status(400).SendString("can't compile bodyparser")
	}

	result, err := h.authSrv.SrvAdminLogin(&login_body)
	if err != nil {
		return c.Status(400).SendString("can't find this user")
	}
